...
const getAchievementIcon = (code: string) => {
  if (code.includes('STREAK')) return <Flame className="h-5 w-5 text-warning" />;
  if (code.includes('FOCUS') || code.includes('BRAIN')) return <Brain className="h-5 w-5 text-ai" />;
  if (code.includes('MORNING') || code.includes('EARLY')) return <Sunrise className="h-5 w-5 text-amber-500" />;
  if (code.includes('FORGE') || code.includes('SCORE')) return <Zap className="h-5 w-5 text-primary" />;
  if (code.includes('GOAL') || code.includes('CHAMPION')) return <Trophy className="h-5 w-5 text-yellow-500" />;
