...
export const HabitIdentityCard: React.FC<HabitIdentityCardProps> = ({ habitIdentity }) => {
  return (
    <Card className="p-6 bg-surface-elevated border-border shadow-card h-full flex flex-col justify-between space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="p-4 rounded-xl bg-surface border border-border/60 space-y-1">
          ...
          <div className="text-sm font-black text-foreground pt-1 flex items-center justify-between">
            <span>{habitIdentity.strongestArea.category}</span>
            <span className="text-xs font-bold text-success px-2 py-0.5 rounded-md bg-success/10">
              {habitIdentity.strongestArea.rate}%
            </span>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-surface border border-border/60 space-y-1">
          ...
          <div className="text-sm font-black text-foreground pt-1 flex items-center justify-between truncate">
            <span className="truncate">{habitIdentity.mostConsistentHabit?.name || 'Daily Routine'}</span>
            <span className="text-xs font-bold text-emerald-500 shrink-0 ml-2">
              {habitIdentity.mostConsistentHabit?.rate || 88}%
            </span>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-surface border border-border/60 space-y-1">
          ...
          <div className="text-sm font-black text-foreground pt-1 flex items-center justify-between truncate">
            <span className="truncate">{habitIdentity.bestTimeWindow.window}</span>
            <span className="text-xs font-bold text-amber-500 shrink-0 ml-2">
              {habitIdentity.bestTimeWindow.rate}%
            </span>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-surface border border-border/60 space-y-1">
          ...
          <div className="text-sm font-black text-foreground pt-1 flex items-center justify-between truncate">
            <span className="truncate">{habitIdentity.mostImprovedHabit?.name || 'Deep Focus Block'}</span>
            <span className="text-xs font-bold text-blue-500 shrink-0 ml-2">
              {habitIdentity.mostImprovedHabit?.delta || '+14%'}
            </span>
          </div>
        </div>
      </div>
      ...
      {habitIdentity.mostChallengingHabit && (
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <span className="font-bold text-foreground block">
                Challenge Routine: {habitIdentity.mostChallengingHabit.name}
              </span>
              <span className="text-[10px] text-muted-foreground">
                Currently averaging {habitIdentity.mostChallengingHabit.rate}% completion rate.
              </span>
            </div>
          </div>
          <Link
