    ...
    <div className="space-y-6 max-w-7xl mx-auto text-left select-none">
      ...
      <div className="border-b border-border/40 pb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2.5 text-xs font-bold bg-primary text-slate-100 rounded-xl hover:bg-primary-hover transition-all active:scale-[0.98] cursor-pointer shadow-md shrink-0 w-max"
        >
      ...
      {/* Form Card if active */}
      {isCreating && (
        <Card className="bg-[#101622] border border-primary/20 rounded-[14px] p-5 max-w-xl animate-in slide-in-from-top-4 duration-200">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs font-semibold text-slate-300">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-slate-200">Experiment Name</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full bg-[#080C14] border border-[#1D293D] rounded-xl px-3 py-2 text-slate-200 focus:outline-none focus:border-primary/50 placeholder:text-muted-foreground/50"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-slate-200">Baseline Metric</label>
                <input
                  type="text"
                  required
            ...
            <div className="space-y-1.5">
              <label className="text-slate-200">Hypothesis / Theory</label>
              <textarea
                required
                rows={2}
                ...
                className="w-full bg-[#080C14] border border-[#1D293D] rounded-xl px-3 py-2 text-slate-200 focus:outline-none focus:border-primary/50 placeholder:text-muted-foreground/50 resize-none"
              />
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-slate-200">Duration (Days)</label>
                <input
                  type="number"
                  required
                  value={durationDays}
                  onChange={(e) => setDurationDays(Number(e.target.value))}
                  className="w-full bg-[#080C14] border border-[#1D293D] rounded-xl px-3 py-2 text-slate-200 focus:outline-none focus:border-primary/50"
                />
              </div>
              <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-2.5 pt-2">
              <button
                ...
                className="flex-1 bg-[#151D2C] hover:bg-[#1D293D] text-slate-300 font-bold py-2 rounded-xl text-center cursor-pointer transition-colors"
              >
                Cancel
              </button>
              <button
                type="submit"
                disabled={submitting}
                className="flex-1 bg-primary hover:bg-primary-hover text-slate-100 font-bold py-2 rounded-xl text-center cursor-pointer transition-colors disabled:opacity-50"
              >
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-3 py-16 text-center">
          <div className="h-16 w-16 rounded-2xl bg-[#101622] flex items-center justify-center text-2xl border border-border/10">🧪</div>
          <div>
            <p className="text-sm font-extrabold text-slate-200">No active experiments</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {experiments.map((exp) => (
            <Card
              key={exp.id}
              className={`bg-[#101622] border rounded-[14px] p-5 flex flex-col justify-between gap-4 transition-colors ${
                exp.status === 'active'
            ...
            >
              <div className="space-y-1.5 text-xs font-semibold">
                <div className="flex justify-between items-center">
                  <span className="text-slate-200 text-sm font-extrabold truncate max-w-[200px]">
                    {exp.name}
                  </span>
                ...
                <div className="py-2.5 border-t border-b border-border/5 text-slate-300 font-medium leading-relaxed">
                ...
                <div className="flex items-center justify-between pt-1">
                  <span>Baseline: <strong className="text-slate-200">{exp.baselineMetric}</strong></span>
              ...
              {exp.status === 'active' && (
                <div className="flex items-center gap-2 pt-2 text-xs font-bold">
                  <button
                    ...
                    className="flex-1 bg-[#10B981]/15 border border-[#10B981]/20 text-[#10B981] hover:bg-[#10B981]/25 py-1.5 rounded-lg flex items-center justify-center gap-1 cursor-pointer transition-colors"
                  >
